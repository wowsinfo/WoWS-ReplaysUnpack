...
Use the Encrypt_* methods to encrypt the plaintext in the mode that you want.
...
internal class Blowfish
{
	...
	private uint[] bfS2 = default!;
	private uint[] bfS3 = default!;

	private uint[] bfP = default!;
	...
	private byte[] initVector = default!;
	private bool iVSet;
	...
	{
		InitializationVector = HexToByte(ct[..16]);
		return Encoding.ASCII.GetString(Decrypt_CBC(HexToByte(ct[16..]))).Replace("\0", "");
	}
	...
	{
		InitializationVector = HexToByte(ct[..16]);
		return Encoding.ASCII.GetString(Crypt_CTR(HexToByte(ct[16..]), 2)).Replace("\0", "");
	}
	...
	private void SetupKey(byte[] cipherKey)
	{
		...
		for (int i = 0; i < 18; i++)
		{
			uint d = (uint)(((key[j % cipherKey.Length] * 256 + key[(j + 1) % cipherKey.Length]) * 256 +
	...
	public byte[] Crypt_CTR(byte[] text, int _)
	{
		if (!iVSet)
		{
	...
	private void SetBlock(byte[] block)
	{
		...
		if (NonStandard)
		{
			xrPar = BitConverter.ToUInt32(block1, 0);
			xlPar = BitConverter.ToUInt32(block2, 0);
		}
		else
		{
			...
			xlPar = BitConverter.ToUInt32(block1, 0);
			xrPar = BitConverter.ToUInt32(block2, 0);
		}
	}
	...
	private void GetBlock(ref byte[] block)
	{
		...
		if (NonStandard)
		{
			block1 = BitConverter.GetBytes(xrPar);
			block2 = BitConverter.GetBytes(xlPar);
		}
		else
		{
			block1 = BitConverter.GetBytes(xlPar);
			block2 = BitConverter.GetBytes(xrPar);
	...
	private void Encipher()
	{
		...
		{
			xrPar = Round(xrPar, xlPar, i + 1);
			xlPar = Round(xlPar, xrPar, i + 2);
		}

		xrPar ^= bfP[17];

		//swap the blocks
		(xlPar, xrPar) = (xrPar, xlPar);
	}
	...
	private void Decipher()
	{
		...
		{
			xrPar = Round(xrPar, xlPar, i);
			xlPar = Round(xlPar, xrPar, i - 1);
		}
	...
	{
		uint x1 = (bfS0[WordByte0(b)] + bfS1[WordByte1(b)]) ^ bfS2[WordByte2(b)];
		uint x2 = x1 + bfS3[WordByte3(b)];
		uint x3 = x2 ^ bfP[n];
		return x3 ^ a;
	}
